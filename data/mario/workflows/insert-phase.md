...
Example: `/mario:insert-phase 72 Fix critical auth bug`
-> after = 72
...
Usage: /mario:insert-phase <after> <description>
Example: /mario:insert-phase 72 Fix critical auth bug
```
...
```bash
INIT=$(mario-tools init phase-op "${after_phase}")
```
...
**Delegate the phase insertion to mario-tools:**

```bash
RESULT=$(mario-tools phase insert "${after_phase}" "${description}")
```
...
`/mario:plan-phase {decimal_phase}`
...
- Don't use this for planned work at end of milestone (use /mario:add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /mario:plan-phase)
...
- [ ] `mario-tools phase insert` executed successfully
