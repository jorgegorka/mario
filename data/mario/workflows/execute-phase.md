...
```bash
INIT=$(mario-tools init execute-phase "${PHASE_ARG}")
```
...
```bash
PLAN_INDEX=$(mario-tools phase-plan-index "${PHASE_NUMBER}")
```
...
1. **Describe what's being built (BEFORE spawning):**
   ...
   **{Plan ID}: {Plan Name}**
   ...
   - Good: "Welcome email sequence with 5-touch nurture flow — onboarding drip, value prop reinforcement, and conversion CTA. Required before paid ads drive traffic to signup."
...
2. **Spawn executor agents:**
   ...
   **Domain routing:** Read `domain` from each plan's frontmatter to determine which executor to spawn:
   ...
   ```bash
   DOMAIN=$(mario-tools frontmatter get "{phase_dir}/{plan_file}" --field domain)
   DOMAIN_GUIDE=$(mario-tools frontmatter get "{phase_dir}/{plan_file}" --field domain_guide)
   ```
...
4. **Report completion — spot-check claims first:**
   ...
   **{Plan ID}: {Plan Name}**
   ...
   - Good: "Email sequences complete — welcome series, lead nurture flow, re-engagement campaign. Paid ads (Wave 3) can now drive traffic to funnels with automated follow-up."

5. **Handle failures:**

   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not an Mario or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
```bash
TEAM_MODE=$(mario-tools config get execution.team 2>/dev/null || echo "false")
```
...
3. **Spawn domain executor agents:** One agent per unique domain in the plans:
   ...
   Task(
     ...
     subagent_type="mario-{domain}-executor",
     model="{executor_model}",
     prompt="
   ...
   ```
   For `general` domain, use `mario-executor` as the subagent_type.
...
5. **Monitor progress:** Orchestrator monitors via `TaskList`. When agents complete tasks:
   ...
   - Cross-domain handoffs: if an email task depends on a strategy task, the email executor can read the strategy SUMMARY.md for context
...
| `gaps_found` | Present gap summary, offer `/mario:plan-phase {phase} --gaps` |
...
`/mario:plan-phase {X} --gaps`

<sub>`/clear` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase}-VERIFICATION.md` — full report
Also: `/mario:verify-work {X}` — manual testing first
```

Gap closure cycle: `/mario:plan-phase {X} --gaps` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/mario:execute-phase {X} --gaps-only` → verifier re-runs.
</step>
...
```bash
mario-tools commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/phases/{phase_dir}/*-VERIFICATION.md .planning/REQUIREMENTS.md
```
</step>
...
<resumption>
Re-run `/mario:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
