...
```bash
INIT=$(mario-tools init progress --include state,roadmap,project,config)
```
...
If missing both ROADMAP.md and PROJECT.md: suggest `/mario:new-project`.
</step>
...
`/mario:discuss-phase {phase}` — gather context and clarify approach
...
**Also available:**
- `/mario:plan-phase {phase}` — skip discussion, plan directly
- `/mario:list-phase-assumptions {phase}` — see Claude's assumptions
...
**Also available:**
- `/mario:execute-phase {phase}` — execute phase plans
- `/mario:verify-work {phase}` — run more UAT testing
...
`/mario:discuss-phase {Z+1}` — gather context and clarify approach
...
**Also available:**
- `/mario:plan-phase {Z+1}` — skip discussion, plan directly
- `/mario:verify-work {Z}` — user acceptance test before continuing
...
**Also available:**
- `/mario:verify-work` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/mario:plan-phase [next]`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/mario:resume-work`
  </step>
...
- [ ] Smart routing: /mario:execute-phase if plans exist, /mario:plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate mario command
      </success_criteria>
