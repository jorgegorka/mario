...
```bash
ROADMAP=$(mario-tools roadmap analyze)
```
...
- Phase 2: Brand Positioning (2/2 plans complete)
- Phase 3: Content Strategy (3/3 plans complete)
- Phase 4: Channel Launch (1/1 plan complete)
...
find . -name "*.md" -o -name "*.txt" -o -name "*.html" | xargs wc -l 2>/dev/null
...
- Files modified: [M]
- Content volume: [count] assets [type]
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  mario-tools summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
**Note:** MILESTONES.md entry is now created automatically by `mario-tools milestone complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
A brand positioning and content strategy system for a B2B SaaS company.

## Core Value

Clear messaging that converts visitors to customers.
...
- Video production — text and image focus
```
...
A brand positioning and content strategy system for a B2B SaaS company with clear messaging and launch-ready web copy.
...
- ✓ Brand positioning document — v1.0
- ✓ Messaging hierarchy — v1.0 (3-tier structure finalized)
- ✓ Website copy — v1.0
...
- Paid advertising — organic first, revisit after v1.1
- Video production — text and image focus
- Podcast — too resource-intensive for launch

## Context

Shipped v1.0 with 12 content assets across web and strategy.
Channels: website, blog, email.
Initial stakeholder feedback showed demand for social media content.
```
...
- 🚧 **v1.1 Engagement** — Phases 5-6 (in progress)
- 📋 **v2.0 Expansion** — Phases 7-10 (planned)
...
- [x] Phase 2: Brand Positioning (2/2 plans) — completed YYYY-MM-DD
- [x] Phase 3: Content Strategy (3/3 plans) — completed YYYY-MM-DD
- [x] Phase 4: Channel Launch (1/1 plan) — completed YYYY-MM-DD
...
| 2. Brand Positioning | v1.0      | 2/2            | Complete    | YYYY-MM-DD |
| 3. Content Strategy  | v1.0      | 3/3            | Complete    | YYYY-MM-DD |
| 4. Channel Launch    | v1.0      | 1/1            | Complete    | YYYY-MM-DD |
| 5. Email Sequences   | v1.1      | 0/1            | Not started | -          |
| 6. Social Media      | v1.1      | 0/2            | Not started | -          |
```
...
**Delegate archival to mario-tools:**

```bash
ARCHIVE=$(mario-tools milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
```bash
INIT=$(mario-tools init execute-phase "1")
```
...
```bash
mario-tools commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
**Names:** Short 1-2 words (v1.0 MVP, v1.1 Engagement, v1.2 Optimization, v2.0 Expansion).
...
- [ ] User knows next step (/mario:new-milestone)
