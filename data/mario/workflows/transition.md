...
**Delegate ROADMAP.md and STATE.md updates to mario-tools:**

```bash
TRANSITION=$(mario-tools phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `mario-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(mario-tools progress bar --raw)
```
...
**Use the transition result from `mario-tools phase complete`:**
...
Exit skill and invoke SlashCommand("/mario:plan-phase [X+1]")
...
**Also available:**
- `/mario:discuss-phase [X+1]` — gather context first
- `/mario:research-phase [X+1]` — investigate unknowns
- Review roadmap
...
Exit skill and invoke SlashCommand("/mario:complete-milestone {version}")
