...
1. **mario-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **mario-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Use /mario:progress to see available phases.
```
Exit workflow.
...
**Also available:**
- `/mario:plan-phase ${PHASE} --skip-research` — plan without research
...
```bash
mario-tools commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
