...
**Examples:** Wrong messaging, logic errors in content flow, broken cross-references, inconsistent tone, factual errors, missing CTAs, audience mismatch
...
**Examples:** Missing disclaimer/compliance text, no CTA in conversion content, missing brand voice adherence, no audience segmentation, missing channel-specific formatting, no tracking parameters, missing alt text for images

**Critical = required for effective/compliant/professional content.** These aren't "nice-to-haves" — they're quality requirements.
...
**Examples:** Missing referenced content, broken cross-links between documents, missing brand guidelines, missing audience persona data, prerequisite content not yet created, missing template
...
**Examples:** New marketing channel (not additional content in existing channel), major brand repositioning, new audience segment, switching platforms/tools, changing campaign strategy, new content category, breaking changes to existing messaging
...
**See @~/.claude/mario/references/checkpoints.md**
...
```bash
git add content/strategy/brand-positioning.md
...
| Type       | When                                            |
| ---------- | ----------------------------------------------- |
| `feat`     | New content, deliverable, asset                 |
| `fix`      | Content correction, messaging fix               |
| `review`   | Quality review changes (review-first DRAFT)     |
| `refine`   | Content polish, no messaging change             |
...
- Bad: "Email campaign implemented"
...
**1. [Rule 1 - Bug] Fixed inconsistent brand voice in email 3**
- **Found during:** Task 4
- **Issue:** [description]
- **Fix:** [what was done]
