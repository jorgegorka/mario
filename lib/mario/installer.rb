...
module Mario
  class Installer
    ...
    def install
      puts "Mario v#{VERSION} — Installing to #{@target_dir}\n\n"
      ...
      puts "\nDone! Launch Claude Code and run /mario:help."
    end
    ...
    def report_local_patches
      ...
      puts "  Run /mario:reapply-patches to merge them into the new version."
      puts ""
    end
    ...
    def source_manifest_keys
      keys = []
      %w[commands/mario agents guides mario].each do |subdir|
    ...
    def copy_commands
      src = File.join(source_dir, "commands", "mario")
      dest = File.join(@target_dir, "commands", "mario")
      copy_tree(src, dest)
    ...
    def copy_agents
      ...
      Dir[File.join(src, "mario-*.md")].each do |file|
        FileUtils.cp(file, File.join(dest, File.basename(file)))
      end
      count = Dir[File.join(dest, "mario-*.md")].size
    ...
    def copy_content
      src = File.join(source_dir, "mario")
      dest = File.join(@target_dir, "mario")
      copy_tree(src, dest)
      puts "  \u2713 Installed mario/ (workflows, templates, references)"
    end

    def write_version
      dest = File.join(@target_dir, "mario", "VERSION")
